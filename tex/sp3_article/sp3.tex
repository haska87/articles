...
\begin{figure}[h]
	\center{\includegraphics[width=0.6\linewidth]{iaea.png}}\\
...
%В table~\ref{tab:iaea_without_alpha} показаны результаты расчета $\alpha$-спектральной задачи без учета запаздывающих нейтронов при использовании различных сеток и конечных элементов. 
%Эти данные демонстрируют сходимость приближенно вычисляемых собственных значений при сгущении сетки $n$ и при увеличении степени аппроксимирующих полиномов $p$. 
%В table~\ref{tab:iaea_without_alpha_10} приведены результаты первых десяти собственных значений $\alpha$-спектральной задачи~(\ref{1.10}) при мелкой сетке $p=3, n=96$. 
%Сами собственные значения   $\mathrm{Re}\lambda_1^{(\alpha)} \leq \mathrm{Re}\lambda_2^{(\alpha)} \leq ...$ хорошо отделены друг от друга.
%В нашем примере главное собственное значение  меньше остальных и поэтому главная гармоника будет затухать медленнее чем остальные.
%Тем самым выражен регулярный режим работы реактора.
%Сама величина $\alpha = \lambda_1^{(\alpha)}$ определяет амплитуду развития нейтронного поля и непосредственно связана с периодом реактора в регулярном режиме.  
%На рисунках \ref{fig:iaea_without_fun_1}, \ref{fig:iaea_without_fun_2}, \ref{fig:iaea_without_fun_3} показаны собственные функции для SP$_3$ модели.
...
%В таблице \ref{tab:iaea_without_alpha_del} показаны результаты расчета $\alpha$-спектральной задачи с учетом запаздывающих нейтронов при использовании различных сеток и конечных элементов.
%Эти данные демонстрируют сходимость приближенно вычисляемых собственных значений при сгущении сетки $n$ и при увеличении степени аппроксимирующих полиномов $p$. 
%В таблице \ref{tab:iaea_without_alpha_del_10} приведены результаты первых десяти собственных значений $\alpha$-спектральной задачи (\ref{1.9}).
...
%Снова главное собственное значение меншье остальны и поэтому главная гармоника будет затухать медленне остальных.
%На рисунках \ref{fig:iaea_without_fun_del_1}, \ref{fig:iaea_without_fun_del_2}, \ref{fig:iaea_without_fun_del_3} показаны собственные функции для SP$_3$ модели.
...
\subsubsection{Solution of Lambda Modes spectral problem}
%В качестве эталонного решения по диффузионной модели взято 1.00551\cite{vestnik2015}, а по транспортной модели взято решения полученное по коду MCNP4C\cite{ryu2010}.
%Результаты расчета модифицированного теста IAEA-2D с отражателем приведены в таблице \ref{tab:iaea_with_lambda}. 
